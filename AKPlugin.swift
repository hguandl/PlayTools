...
class AKPlugin: NSObject, Plugin {
    ...
    @available(macOS, introduced: 10.5, deprecated: 14.0, obsoleted: 15.0)
    private func legacyWindowImage() -> CGImage? {
        guard let windowID = NSApplication.shared.windows.first?.windowNumber else {
            return nil
        }
        return CGWindowListCreateImage(.null, .optionIncludingWindow, CGWindowID(windowID),
                                       [.bestResolution, .boundsIgnoreFraming])
    }

    // Capture the injected process's own window directly on macOS 14.4 and
    // later. This path does not require Screen Recording permission. Older
    // systems fall back to CGWindowListCreateImage.
    @MainActor
    func captureImage() async -> CGImage? {
        if #available(macOS 14.4, *) {
            do {
                ...
                Self.logger.error("ScreenCaptureKit current-process capture failed: \(error, privacy: .private)")
            }
        }
        return windowImage
    }
